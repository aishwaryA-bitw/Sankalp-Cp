...
export default function AssignTask() {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1gTDJigoeyXYzxR7HxDMwOjGL0hPP3Qc-P9HAChNCxUw";
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1gTDJigoeyXYzxR7HxDMwOjGL0hPP3Qc-P9HAChNCxUw/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName
      )}`;
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1gTDJigoeyXYzxR7HxDMwOjGL0hPP3Qc-P9HAChNCxUw";
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    await fetch(
      "https://script.google.com/macros/s/AKfycbzT9dstlyMKZUK26uGroItx2qSUWLwQ2DeDafLmQ8jvXlyFgao2kGL4EZkTkJ5nwuy77w/exec",
      {
        method: "POST",
        body: formPayload,
        mode: "no-cors",
