...
export default function AdminDashboard() {
  ...
  const fetchDepartmentData = async () => {
    ...
    try {
      ...
      const response = await fetch(`https://docs.google.com/spreadsheets/d/1TSHYw5iEMj1_K1oeVjqMcG6uQRVHQ3Ejj2W0IimJmpg/gviz/tq?tqx=out:json&sheet=${sheetName}`);
      ...
      const processedRows = data.table.rows.map((row, rowIndex) => {
        ...
        if (dashboardType === "delegation") {
          ...
          if (!taskStartDateObj) {
            if (rowIndex <= 5) console.log(`Row ${rowIndex + 1}: Skipped due to invalid date`);
            return null;
          }
        }
  ...
  return (
          ...
          <div className="rounded-lg border border-l-4 border-l-blue-500 shadow-md hover:shadow-lg transition-all bg-white">
            ...
            <div className="p-4">
              <div className="text-3xl font-bold text-blue-700">
                {getTasksByView("recent").length + getTasksByView("overdue").length}
              </div>
              <p className="text-xs text-blue-600">
                Total Tasks (Till date)
              </p>
            </div>
          </div>
          ...
          <div className="rounded-lg border border-l-4 border-l-green-500 shadow-md hover:shadow-lg transition-all bg-white">
            ...
            <div className="p-4">
              ...
              <p className="text-xs text-green-600">
                {dashboardType === "delegation" ? "Tasks completed once" : "All completed tasks"}
              </p>
            </div>
          </div>
    ...
    <div className="p-4">
      <div className="text-3xl font-bold text-amber-700">
        {dashboardType === "delegation" ? departmentData.completedRatingTwo : getTasksByView("recent").length}
      </div>
      <p className="text-xs text-amber-600">
        {dashboardType === "delegation" ? "Tasks completed twice" : "Pending Data (Today)"}
      </p>
    </div>
  </div>
                  ...
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    <div className="space-y-2">
                      ...
                      {dashboardType === "delegation" ? (
                        ...
                        <>
                          <p className="text-xs text-purple-600">All tasks including future dates</p>
                          {dateRange.filtered && (
                    ...
                    <div className="space-y-2">
                      ...
                      <p className="text-xs text-purple-600">All completed tasks</p>
                    </div>
                    <div className="space-y-2">
                      ...
                      <div className="rounded-lg border border-purple-200 bg-white p-4">
                        ...
                        <p className="text-xs text-purple-600 mt-2">
                          {dashboardType === "delegation" ?
                            ...
                            `${dateRange.filtered ? filteredDateStats.completedTasks : departmentData.completedTasks} of ${dateRange.filtered ? filteredDateStats.totalTasks : departmentData.totalTasks} tasks completed (including future dates)`
                          }
                        </p>
                      </div>
                    </div>
